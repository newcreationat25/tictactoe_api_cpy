...
// POST /generate-token
appRoute.post('/authenticate', async (req, res) => {
...
appRoute.post('/login', exports.authenticateJWT, async (req, res) => {
    const responseObj = new response_1.reponse();
    try {
        ...
        const existing = await db_1.default.select().from(user_1.users).where((0, drizzle_orm_1.eq)(user_1.users.email, email));
